/*
 * sample_anycard.c - Sample code using any FeliCa card.
 *
    ...
    char* argv[])
{
    ...
    printf("FeliCa Request Response and Request System Code sample.\n");
     ...
     */
    {
        ...
        command_timeout =
            FELICA_CC_CALC_TIMEOUT(card.pmm[FELICA_CC_PMM_REQUEST_RESPONSE],
                                   0);
        command_timeout += FELICA_COMMAND_TIMEOUT_ADD;

        printf("start Request Response ...\n");

        for (nretries = 0; nretries <= s_command_max_retry_times; nretries++) {
            printf("  calling felica_cc_request_response() ...\n");
            rc = felica_cc_request_response(&devf,
                                            &card,
                                            &mode,
                                            command_timeout);
            if ((rc != ICS_ERROR_TIMEOUT) &&
        ...
        if (rc != ICS_ERROR_SUCCESS) {
            fprintf(stderr,
                    "    failure in felica_cc_request_response():%u\n", rc);
            finalize(&dev);
            return -1;
        }
     ...
     */
    {
        ...
        command_timeout =
            FELICA_CC_CALC_TIMEOUT(card.pmm[FELICA_CC_PMM_REQUEST_SYSTEM_CODE],
                                   0);
        command_timeout += FELICA_COMMAND_TIMEOUT_ADD;

        printf("start Request System Code ...\n");

        for (nretries = 0; nretries <= s_command_max_retry_times; nretries++) {
            printf("  calling felica_cc_request_system_code() ...\n");
            rc = felica_cc_request_system_code(&devf,
                                               &card,
                                               32,
                                               &num_of_system_codes,
                                               system_code_list,
        ...
        if (rc != ICS_ERROR_SUCCESS) {
            fprintf(stderr,
                    "    failure in felica_cc_request_system_code():%u\n", rc);
            finalize(&dev);
            return -1;
        }
