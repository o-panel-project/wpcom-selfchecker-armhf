...
#define MY_VERSION      "Polling (Type B) for RC-S956 --- ver 1.0"
#define MY_COPYRIGHT    "Copyright 2009 Sony Corporation"
...
static UINT32 s_interval = DEFAULT_INTERVAL;

static int polling_rcs956_typeb(void)
{
    UINT32 rc;
    ...
    for (n_loops = 0;
        ...
        for (n_cycles = 0; n_cycles < N_CYCLES_PER_LOOP; n_cycles++) {
            ...
            for (n_pollings_in_cycle = 0;
                ...
                /* Type B polling */
                rc = rcs956_typeb_polling(&rcs956,
                                          0x00,
                                          &card, &attrib,
                                          s_timeout);
                if (rc == ICS_ERROR_SUCCESS) {
    ...
    char* argv[])
{
    ...
    for (i = 1; i < argc; i++) {
        if (((strcmp(argv[i], "-p") == 0) ||
            ...
            i++;
            s_interval = strtoul(argv[i], NULL, 0);
        } else if (argv[i][0] == '-') {
            print_usage(argv[0]);
            return -1;
        } else {
