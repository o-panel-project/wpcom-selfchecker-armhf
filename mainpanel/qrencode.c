...
#if	defined(_QRENCODE_API_)
int qrencode2png(char *outfile, unsigned char *intext)
#else
...
#endif
{
	...
	if(intext == NULL) {
		...
		if (intext == NULL) {
			fprintf(stderr, "Input file %s, data nothing.\n", infile);
			return(EXIT_FAILURE);
		}
...
#if defined(_QRENCODE_API_) && defined(_QRENCODE_TEST_)
int main(int argc, char **argv)
{
	int i_opt;
	char *outfile = NULL;
	...
	while ((i_opt = getopt(argc, argv, "o:")) != -1) {
		switch (i_opt) {
		case 'o':
			outfile = optarg;
			break;
	...
	return qrencode2png(outfile, intext);
}
#endif

#if 0	/* How to build. */
/* call from mainpanel. (function) */
/*
   arm-linux-gnueabihf-gcc -c qrencode.c \
   -D_QRENCODE_API_ -DHAVE_CONFIG_H -I../libqrencode/include

   extern int qrencode2png(char *outfile, unsigned char *intext);
 */

/* unit test with main. (standalone) */
/*
   arm-linux-gnueabihf-gcc -o qrencode_test qrencode.c \
   -D_QRENCODE_API_ -D_QRENCODE_TEST_ -DHAVE_CONFIG_H \
   -I../libqrencode/include -I/opt/arm-dev/armhf/usr/include \
   -L/opt/arm-dev/armhf/usr/lib --sysroot=/opt/arm-dev/armhf \
   ../libs.armhf/libqrencode.a -lpng
 */
#endif
